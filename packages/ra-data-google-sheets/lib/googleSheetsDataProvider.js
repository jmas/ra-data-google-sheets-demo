...
exports.googleSheetsDataProvider = void 0;
const formatDataForGoogleSheets_1 = require("./formatDataForGoogleSheets");
const formatDataForReactAdmin_1 = require("./formatDataForReactAdmin");
const googleSheetsDataProvider = (spreadsheetId) => {
    const getResourceRows = async (resource, params = {}) => {
        const response = await window.gapi.client.sheets.spreadsheets.values.get({
            spreadsheetId,
            range: resource,
        });
        const sheet = (0, formatDataForReactAdmin_1.processSheet)(response.result.values);
        if (params && params.pagination) {
            ...
            return Object.assign(Object.assign({}, sheet), { data: paginatedValues });
        }
        return sheet;
    };
    ...
    const countResourceRows = async (resource) => {
        ...
        return (0, formatDataForReactAdmin_1.processSheet)(response.result.values).total;
    };
    ...
    return {
        create: async (resource, { data: formData }) => {
            ...
            const values = (0, formatDataForGoogleSheets_1.processForm)(headers, newId, formData);
        ...
        update: async (resource, { id, data: formData }) => {
            const headers = await getResourceHeaders(resource);
            const values = (0, formatDataForGoogleSheets_1.processForm)(headers, id, formData);
...
exports.googleSheetsDataProvider = googleSheetsDataProvider;
